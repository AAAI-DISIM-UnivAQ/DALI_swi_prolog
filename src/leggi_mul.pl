...
testa_multiplo_evento(Head,Me,F):-arg(2,Head,A2),(var(A2) -> scrivi_clause_no_mul(Me,F) ; (functor(A2,Fu,_),if(Fu=eve,testa_multiplo_evento2(Head,Me),
...
ejec_mul(X,Me):-(var(X) -> true ; (functor(X,F,N),if((X=[];N\=1),true,no_eve_mult(X,Me,F)))).
