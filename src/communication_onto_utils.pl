...
fetch_ask(Response,Bool):-
        member(content(C),Response), atom_to_term(C, XmlParsed, _),
...
create_query_url(Query,Prefixes,Repository,QueryCodes):-
   ...
   uri_query_components(UrlArgs, [query=PQ]),
...
do_query(QueryCodes,HostPort,Response):-
        ...
        atomic_list_concat(['http://', Host, ':', Port, QueryCodes], Url_), http_open(Url_, ResponseStream_, [request_header('Accept'='application/sparql-results+xml')]), read_string(ResponseStream_, _, ResponseStr_), close(ResponseStream_), Response = [content(ResponseStr_)].
