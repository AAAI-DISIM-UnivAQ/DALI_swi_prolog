...
%% Suppress noisy warnings during DALI file loading
:- multifile user:message_hook/3.
user:message_hook(check(singleton(_Vars)), warning, _) :- !.
user:message_hook(redefined_procedure(_, _), warning, _) :- !.

%% SWI-Prolog compatibility layer - must be loaded first for if/3
:- ['swi_compat.pl'].
...
%% Linda client implementation for SWI-Prolog
:- use_module(library(socket)).
...
bagof_rd_noblock(Template, Goal, Bag) :- linda_request(bagof_rd_noblock(Template, Goal, Bag), Reply), Reply = found(Bag).

% fdbg_assign_name/2 is provided by swi_compat.pl

:-dynamic eve/1.
:-dynamic eve_cond/1.
:-['utils.pl'].

start0(FI):-style_check(-discontiguous),
            style_check(-discontiguous),
...
start1(Fe,AgentName,Libr,Fil):-
  ...
  load_files(FilePlv,[silent(true),redefine_module(true)]),
...
appendi_files(T):-style_check(-discontiguous),style_check(-singleton),
  print(T),nl,
...
man_go:-setof(X,clause_man(X),L),
        ...
        % Cio se cia esattamente un argomento
...
send_message(E,AgM):-clause(ext_agent(AgM,IndM,Ontology,_),_),
             ...
             get_time(_WallStamp1_), Time is truncate(_WallStamp1_*1000),
...
proc_rip_esterni(Me,Y,Stream):-
        ...
        clause2_esterni(X,Stream):-nl(Stream),write(Stream,'eve_cond('),write(Stream,X),
                    ...
                    write(Stream,'cd('),write(Stream,X),write(Stream,'):-true.'),nl(Stream),
                    write(Stream,'eve('),write(Stream,X),write(Stream,'):-true.').
...
%% Il do_action  asserivo nel plv quando nelle azioni.
...
keep_action1:-findall(azione(X,Ag),clause(do_action(X,Ag),_),L),
         last(L,U),
                  repeat,
                           ...
                           get_time(_WallStamp2_), Te is truncate(_WallStamp2_*1000),
...
add_evento(X):-get_time(_WallStamp3_), Tc is truncate(_WallStamp3_*1000),assert(past(X,Tc,program)).
...
ev_intp:-findall(Y,tempI(Y),Ls),
         get_time(_WallStamp4_), Tc is truncate(_WallStamp4_*1000),
         last(Ls,La),
         repeat,
           member(Me,Ls),
...
controlla_freq_tent:-clause(agente(_,_,S,_),_),get_time(_WallStamp5_), T is truncate(_WallStamp5_*1000),
                  leggiriga(S,2),clause(evintI(L),_),
...
controlla_freq_iv:-clause(agente(_,_,S,_),_),get_time(_WallStamp6_), T is truncate(_WallStamp6_*1000),
                  leggiriga(S,2),clause(evintI(L),_),
...
processa_tempo_past(Me,T):-arg(1,Me,P),arg(2,Me,Tc),arg(3,Me,Pr),T1 is T*1000, Tt is Tc+T1,
          get_time(_WallStamp7_), Tcorr is truncate(_WallStamp7_*1000),
...
processa_tempo_rem(Me,T):-arg(1,Me,P),arg(2,Me,Tc),T1 is T*1000, Tt is Tc+T1,
          get_time(_WallStamp8_), Tcorr is truncate(_WallStamp8_*1000),
...
pari:-sleep(1),random_between(1,10,R), R1 is R mod 2,
...
%% Il file PLV non  vuoto. Contiene cio che  scritto nel file PL.
...
callasp(Name,Model):-name(Name,L),append(L,[46,116,120,116],Lf),
                ...
                name(Ff,Lp4),
                append(Lp5f,Lp3,Lpf),name(T,Lpf),shell(T),assert(wait_asp_result(Ff,Name)).
                  ...
                  last(L1,U),
                          repeat,
                                member(M,L1),
                                   ...
                                   get_time(_WallStamp9_), Tc is truncate(_WallStamp9_*1000),
...
empty_plan_generated(Name):-get_time(_WallStamp10_), Tc is truncate(_WallStamp10_*1000),
...
save_on_log_file(P):-clause(agent(N),_),
        ...
        catch((make_directory_path('log'),open(Q,append,Stream,[]),
...
acquire_knowledge(Context, PrologFact) :-
    print('>>> [SYSTEM] INTERROGO ORACOLO AI...'), nl,
        ...
        (
            tcp_socket(AISock), tcp_connect(AISock, '127.0.0.1':9000), tcp_open_socket(AISock, AIIn, AIOut),
            set_stream(AIOut, encoding(utf8)), set_stream(AIIn, encoding(utf8)),
            
            write(AIOut, Context), write(AIOut, '.'), nl(AIOut),
            flush_output(AIOut),
            
            read(AIIn, PrologFact),
            close(AIIn), close(AIOut),
        ...
        (
            ...
            PrologFact = suggestion(nessun_consiglio)
        )
    ).